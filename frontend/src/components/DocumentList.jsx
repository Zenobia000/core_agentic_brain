...
import { FileText, Trash2, Check, RefreshCw, Upload, Eye } from 'lucide-react';
...
export default function DocumentList({ 
  ...
  const fetchDocuments = useCallback(async () => {
    try {
      ...
      // 🆕 只在首次載入時自動選取全部已索引文件
      if (isFirstLoad.current && data.documents.length > 0) {
        const indexedDocs = data.documents.filter(d => d.indexed).map(d => d.name);
        if (indexedDocs.length > 0 && selectedDocs.length === 0) {
  ...
  const handlePreview = (doc, e) => {
    e.stopPropagation();
    if (!doc.indexed) return;
    
    setPreviewingDoc(doc.name);
    if (onPreviewPdf) {
      onPreviewPdf(doc.name);
    }
  };

  // 勾選 Checkbox
  const toggleDocument = (docName, e) => {
    e.stopPropagation();
        ...
        {documents.length === 0 ? (
          ...
          documents.map((doc) => (
            <div
              key={doc.name}
              className={`
                group relative flex items-center gap-2 px-2 py-2 rounded-lg transition-all duration-150
                ${!doc.indexed ? 'opacity-50' : ''}
                ${previewingDoc === doc.name
            ...
            >
              {/* Checkbox - 選擇搜尋範圍 */}
              <div 
                onClick={(e) => doc.indexed && toggleDocument(doc.name, e)}
                className={`
                  w-4 h-4 rounded flex items-center justify-center flex-shrink-0 transition-all cursor-pointer
                  ${selectedDocs.includes(doc.name)
                    ? 'bg-violet-500 text-white'
                    : 'bg-white/10 border border-white/20 hover:border-violet-400'
                  }
                  ${!doc.indexed ? 'cursor-not-allowed' : ''}
                `}
              >
                {selectedDocs.includes(doc.name) && (
              ...
              {/* 文件資訊 - 點擊預覽 */}
              <div 
                className={`flex-1 min-w-0 ${doc.indexed ? 'cursor-pointer' : 'cursor-wait'}`}
                onClick={(e) => handlePreview(doc, e)}
              >
                <div className="flex items-center gap-1.5">
                  <p className="text-sm text-white/80 truncate hover:text-white transition-colors" title={doc.name}>
                    {doc.name}
                  </p>
                  {doc.indexed && (
                    <Eye className="w-3 h-3 text-white/20 opacity-0 group-hover:opacity-100 transition-opacity" />
                  )}
                </div>
