...
from orchestrator.actors.base import Actor, ActorMessage
from core.protocols import Task, Intent
    ...
    def __init__(self, name: str = "planner", config: Optional[Dict[str, Any]] = None):
        ...
        self.available_tools = {
            ...
            "execute_python": {
                ...
                "parameters": ["code", "file"]
            },
            "git_clone": {
                "service": "repo_ops",
                "description": "Clone Git 倉庫",
                "parameters": ["url", "path"]
            },
            "git_status": {
                "service": "repo_ops",
...
1. **始終生成多個查詢**: 對於任何問題，至少生成 2-3 個不同角度的搜尋查詢
2. **使用 rag_search_multiple**: 當需要多角度搜尋時，使用此工具傳入多個 queries
3. **使用 rag_ask**: 當用戶需要綜合回答時（如總結、比較、解釋）
4. **查詢要具體**: 避免太模糊的查詢，加入具體的關鍵詞
5. **中英混合**: 對於技術問題，同時使用中英文關鍵詞
...
{{
    ...
    "tasks": [
        {{
            "id": "task_1",
            "tool": "rag_search_multiple 或 rag_ask",
            "parameters": {{
                "queries": ["查詢1", "查詢2", "查詢3"],
                "question": "如果使用 rag_ask 則填入完整問題",
                "top_k": 5
            }},
            "dependencies": [],
            "description": "任務說明"
