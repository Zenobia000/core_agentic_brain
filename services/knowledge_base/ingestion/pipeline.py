...
class Indexer:
    ...
    def _init_clients(self):
        ...
        if self.qdrant_client is None:
            ...
            if not api_key:
                logger.error(f"❌ OPENAI_API_KEY 未設置！.env 路徑: {_env_path}, 存在: {_env_path.exists()}")
                raise ValueError(f"OPENAI_API_KEY 未設置。請確認 {_env_path} 檔案存在且包含 OPENAI_API_KEY=sk-xxx")
